...
void bpnn_layerforward_ocl_MBAP(float *input_cuda,
...
void bpnn_adjust_weights_ocl_MBAP( __global float * delta,   
										 int hid,         
										__global float * ly,      
										 int in,          
										__global float * w,       
...
int bpnn_train_kernel(BPNN *net, float *eo, float *eh)
{
	...
	float *output_hidden;	//no need to assign memory
	
	input_weights_one_dim = (float *) aligned_alloc(4096, (in + 1)* (hid + 1) * sizeof(float));
	input_weights_prev_one_dim = (float *) aligned_alloc(4096, (in + 1)* (hid + 1) * sizeof(float));
	partial_sum = (float *) aligned_alloc(4096, num_blocks * WIDTH * sizeof(float));
	...
	input_ocl = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, (in + 1) * sizeof(float), (void*)net->input_units, &err );
	if(err != CL_SUCCESS) { printf("ERROR: clCreateBuffer input_ocl\n"); return -1;}
	input_hidden_ocl = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, (in + 1) * (hid + 1) * sizeof(float), (void*)input_weights_one_dim, &err );
	if(err != CL_SUCCESS) { printf("ERROR: clCreateBuffer input_hidden_ocl\n"); return -1;}
	output_hidden_ocl = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, (hid + 1) * sizeof(float), (void*)output_hidden, &err );
	if(err != CL_SUCCESS) { printf("ERROR: clCreateBuffer output_hidden_ocl\n"); return -1;}
	hidden_partial_sum = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, num_blocks * WIDTH * sizeof(float), (void*)partial_sum, &err );
	if(err != CL_SUCCESS) { printf("ERROR: clCreateBuffer hidden_partial_sum\n"); return -1;}
	hidden_delta_ocl = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, (hid + 1) * sizeof(float), (void*)net->hidden_delta, &err );
	if(err != CL_SUCCESS) { printf("ERROR: clCreateBuffer hidden_delta_ocl\n"); return -1;}
	input_prev_weights_ocl = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, (in + 1) * (hid + 1) * sizeof(float), (void*)input_weights_prev_one_dim, &err );
