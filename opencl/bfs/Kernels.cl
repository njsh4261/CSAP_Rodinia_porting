...
__kernel void BFS_1_throttling( const __global Node* g_graph_nodes,
								const __global int* g_graph_edges, 
								__global char* g_graph_mask, 
								__global char* g_updating_graph_mask, 
								__global char* g_graph_visited, 
								...
								int throttling_alloc)
{
	...
	if (get_local_id(0) % throttling_mod < throttling_alloc) {
        for (int dynamic_work = atomic_inc(local_worklist);
            ...
            int global_id_0 = get_group_id(0) * get_local_size(0) + get_global_offset(0) + dynamic_work;
			int tid = global_id_0;
			if( tid<no_of_nodes && g_graph_mask[tid]){
				g_graph_mask[tid]=false;
				for(int i=g_graph_nodes[tid].starting; i<(g_graph_nodes[tid].no_of_edges + g_graph_nodes[tid].starting); i++){
...
__kernel void BFS_2_throttling(__global char* g_graph_mask, 
							   __global char* g_updating_graph_mask, 
							   __global char* g_graph_visited, 
							   __global char* g_over,
							   const int no_of_nodes,
