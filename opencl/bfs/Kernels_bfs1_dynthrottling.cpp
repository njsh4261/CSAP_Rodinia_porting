...
extern "C" {
    int CPU_thread_entry(int id) {
        ...
        std::atomic_int* worklist = (std::atomic_int*)x86_kernel_params[num_total_params * id + 9];
        size_t num_tasks = (size_t)x86_kernel_params[num_total_params * id + 10];

        BFS_1(g_graph_nodes, g_graph_edges, g_graph_mask, g_updating_graph_mask, g_graph_visited,
            g_cost, no_of_nodes, global_size, local_size, worklist, num_tasks);
