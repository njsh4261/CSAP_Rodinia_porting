...
void run_bfs_gpu(int no_of_nodes, Node *h_graph_nodes, int edge_list_size, \
	...
	try{
		...
		do{
			...
			// StartProfiling();
			_clInvokeKernel(kernel_id, no_of_nodes, work_group_size);			
			// StopProfiling("bfs_kernel", "profiling.txt");
...
int main(int argc, char * argv[])
{
	...
	try{
		//FIXME : There should be second argument representing the size of work group
		char *input_f;
		if(argc!=2){
		  Usage(argc, argv);
		  exit(0);
		...
		h_graph_nodes = (Node *) memalign(4096, sizeof(Node)*no_of_nodes);
		h_graph_mask = (char*) memalign(4096, sizeof(char)*no_of_nodes);
		h_updating_graph_mask = (char*) memalign(4096, sizeof(char)*no_of_nodes);
		h_graph_visited = (char*) memalign(4096, sizeof(char)*no_of_nodes);
    	...
    	fscanf(fp,"%d",&edge_list_size);
   		int id,cost;
		int* h_graph_edges = (int*) memalign(4096, sizeof(int)*edge_list_size);
		...
		int	*h_cost = (int*) memalign(4096, sizeof(int)*no_of_nodes);
		int *h_cost_ref = (int*) memalign(4096, sizeof(int)*no_of_nodes);
		for(int i=0;i<no_of_nodes;i++){
			h_cost[i]=-1;
			h_cost_ref[i] = -1;
		}
