...
// To enable profiler, decommentize following line and type 'make profile' at the terminal
// #define PROFILER
#ifdef PROFILER
...
int main(int argc, char *argv [])
{
	...
	// if(argc==3){
	if(argc==6){
		num_cpu_cores = atoi(argv[1]);
		throttling_mod = atoi(argv[2]);
		throttling_alloc = atoi(argv[3]);
		if( (num_cpu_cores = atoi(argv[1])) <= 0 ||
			(throttling_mod = atoi(argv[2])) <= 0 ||
			(throttling_alloc = atoi(argv[3])) <= 0 ){
			fprintf(stderr, "Invalid MBAP parameters\n");
			return 0;
		}
		...
		printf("Provide MBAP parameters and boxes1d argument, example: 1 1 1 -boxes1d 16");
		return 0;
	}
