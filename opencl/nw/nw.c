...
void usage(int argc, char **argv)
{
	...
	fprintf(stderr, "Usage: %s <max_rows/max_cols> <penalty> <num_cpu_cores> <throttling_mod> <throttling_alloc>\n", argv[0]);
	fprintf(stderr, "\t<dimension>  - x and y dimensions\n");
	fprintf(stderr, "\t<penalty> - penalty(positive integer)\n");
	fprintf(stderr, "\t<file> - filename\n");
	fprintf(stderr, "\t<num_cpu_cores> - filename\n");
	fprintf(stderr, "\t<throttling_mod> - filename\n");
	fprintf(stderr, "\t<throttling_alloc> - filename\n");
	exit(1);
}
...
int main(int argc, char **argv){
  ...
  printf("WG size of kernel = %d \n", BLOCK_SIZE);
	...
	reference = (int *)memalign(4096, max_rows * max_cols * sizeof(int) );
    input_itemsets = (int *)memalign(4096, max_rows * max_cols * sizeof(int) );
	output_itemsets = (int *)memalign(4096, max_rows * max_cols * sizeof(int) );
	...
	input_itemsets_d = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, max_cols * max_rows * sizeof(int), (void*)input_itemsets, &err );
	if(err != CL_SUCCESS) { printf("ERROR: clCreateBuffer input_item_set (size:%d) => %d\n", max_cols * max_rows, err); return -1;}
	reference_d		 = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, max_cols * max_rows * sizeof(int), (void*)reference, &err );
	if(err != CL_SUCCESS) { printf("ERROR: clCreateBuffer reference (size:%d) => %d\n", max_cols * max_rows, err); return -1;}
	output_itemsets_d = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, max_cols * max_rows * sizeof(int), (void*)output_itemsets, &err );
	if(err != CL_SUCCESS) { printf("ERROR: clCreateBuffer output_item_set (size:%d) => %d\n", max_cols * max_rows, err); return -1;}
