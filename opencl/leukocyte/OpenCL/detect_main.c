...
int main(int argc, char ** argv) {
	...
	if (argc !=6)	{
		fprintf(stderr, "Usage: %s <num_cpu_cores> <throttling_mod> <throttling_alloc> <input file> <number of frames to process>", argv[0]);
		exit(EXIT_FAILURE);
	}
	...
	if( (num_cpu_cores = atoi(argv[1])) <= 0 ||
      ...
      (throttling_alloc = atoi(argv[3])) <= 0 ){
		fprintf(stderr, "Invalid MBAP arguments. Exit the program.\n");
		exit(EXIT_FAILURE);
	}

	// Let the user specify the number of frames to process
	int num_frames = atoi(argv[5]);
	
	// Open video file
	char *video_file_name = argv[4];
