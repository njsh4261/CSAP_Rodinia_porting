...
static int allocate(int Nparticles, int countOnes){
	...
	arrayX_GPU = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, sizeof (double) *Nparticles, (void*)arrayX, &err);
	if (err != CL_SUCCESS) {
		printf("ERROR: clCreateBuffer arrayX_GPU (size:%d) => %d\n", Nparticles, err);
		return -1;
	}
	arrayY_GPU = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, sizeof (double) *Nparticles, (void*)arrayY, &err);
	if (err != CL_SUCCESS) {
		printf("ERROR: clCreateBuffer arrayY_GPU (size:%d) => %d\n", Nparticles, err);
		return -1;
	}
	xj_GPU = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, sizeof (double) *Nparticles, (void*)xj, &err);
	if (err != CL_SUCCESS) {
		printf("ERROR: clCreateBuffer xj_GPU (size:%d) => %d\n", Nparticles, err);
		return -1;
	}
	yj_GPU = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, sizeof (double) *Nparticles, (void*)yj, &err);
	if (err != CL_SUCCESS) {
		printf("ERROR: clCreateBuffer yj_GPU (size:%d) => %d\n", Nparticles, err);
		return -1;
	}
	CDF_GPU = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, sizeof (double) *Nparticles, (void*)CDF, &err);
	if (err != CL_SUCCESS) {
		printf("ERROR: clCreateBuffer CDF_GPU (size:%d) => %d\n", Nparticles, err);
		return -1;
	}
	u_GPU = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, sizeof (double) *Nparticles, (void*)u, &err);
	if (err != CL_SUCCESS) { printf("ERROR: clCreateBuffer u_GPU (size:%d) => %d\n", Nparticles, err); return -1; }
